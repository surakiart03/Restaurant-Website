 ...
 *
 * @uses food_restro_header_style()
 */
function food_restro_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'food_restro_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'fff',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'food_restro_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'food-restro' ),
		),
	) );
}
add_action( 'after_setup_theme', 'food_restro_custom_header_setup' );

if ( ! function_exists( 'food_restro_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see food_restro_custom_header_setup().
	 */
	function food_restro_header_style() {
		...
		wp_add_inline_style( 'food-restro-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'food_restro_header_style', 10 );